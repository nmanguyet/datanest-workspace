...
spark = init_spark3.setup(
    ...
    script_name=f'2_3_12week_fts_data_charge_by_hour_{extract_date_str}'
)

input_dir = 'hdfs://datanest-ha/feature/weekly/data_charge_fts_agg_l1w'

out_dir = 'hdfs://datanest-ha/feature/lxm/data_charge_fts_agg_by_hour_l12w'

### a. weekly
print("extraction date", extract_date_str)
extract_date_dt = datetime.strptime(extract_date_str, "%Y-%m-%d")

lb_date_dt = extract_date_dt - timedelta(days=11 * 7)
...
fts_agg_l12w = (
    data_charge_df_l12w.groupBy('phone_number', 'is_weekend', 'hour')
    .agg(
        F.avg('data_charge_updata_hour_l1w').alias(f'data_charge_avg_updata_per_hour_wk_{lxw_str}'),  ###l12w
        F.avg('data_charge_downdata_hour_l1w').alias(f'data_charge_avg_downdata_per_hour_wk_{lxw_str}'),  ###l12w
    )
)

range_hour = [4, 6, 7]
fts_agg_l12w_hour = (
    fts_agg_l12w.groupBy('phone_number')
    .agg(
        *(F.sum(F.when(F.col('hour') == i, (F.col(f'data_charge_avg_updata_per_hour_wk_{lxw_str}'))))
          .alias(f'data_charge_updata_at_{i}_per_wk_{lxw_str}') for i in set(range_hour)),
        *(F.sum(F.when(F.col('hour') == hour, (F.col(f'data_charge_avg_downdata_per_hour_wk_{lxw_str}'))))
          .alias(f'data_charge_downdata_at_{hour}_{lxw_str}') for hour in set(range_hour)),
    )
)

# print
print("writing to file")
fts_agg_l12w_hour.write.mode('overwrite').parquet(f'{out_dir}/date={extract_date_str}')
print(datetime.now())
